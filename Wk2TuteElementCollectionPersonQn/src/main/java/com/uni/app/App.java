...
@SpringBootApplication public class App {
   ...
   @Transactional @Data @Component class AppInit implements ApplicationRunner {
   @NonNull private final PersonRepository personRepository; // DI via lombok constructor
   @Override public void run(ApplicationArguments args) throws Exception {
     Person ann = new Person();
     ann.setName("Ann");
     ann.getNumbers().add("041838383");
     ann.getNumbers().add("041838382");
     ann.getNumbers().add("041838382"); // duplicate
     personRepository.save(ann);
     List<Person> people = personRepository.findAll();
     for (Person p : people) { 
         System.out.print(p.getName()+":");   
         for (String ph : p.getNumbers()) { System.out.print(ph+";"); }
         System.out.println();   }   }   }  }
// jpa_bi
@Component
